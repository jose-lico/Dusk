		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				if(scriptData)
				{
					...
					s.luaScriptHandle = scene->m_AssetHandler->AddToLuaScriptPool2(entity["Entity"].as<uuids::uuid>(), script);
					scene->m_AssetHandler->LuaScriptPool2(s.luaScriptHandle)->m_Ent = deserializedEntity;
				}
			}
		}
